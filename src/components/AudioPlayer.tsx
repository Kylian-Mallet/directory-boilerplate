...
export default function AudioPlayer({ audioUrl }: AudioPlayerProps) {
  ...
  return (
    <Card className="my-8 p-4 bg-transparent w-full border">
      <h2 className="text-2xl font-bold mb-1 font-heading">{siteTexts.audio.listenToStory}</h2>
      <div className="bg-background rounded-lg p-4 w-full">
        <div className="flex items-center space-x-4">
          <Button variant="ghost" size="icon" onClick={togglePlayPause} aria-label={isPlaying ? siteTexts.audio.pause : siteTexts.audio.play}>
            {isPlaying ? (
